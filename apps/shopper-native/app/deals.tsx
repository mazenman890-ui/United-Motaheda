          ...
          <LinearGradient colors={["#1A0000", "#3D0000"]} style={s.heroBg}>
            ...
            <View style={s.heroInner}>
              <View style={s.heroImg}>
                {product.imageUrl ? (
                  <Image source={{ uri: product.imageUrl }} style={{ flex: 1 }} contentFit="contain" transition={180} />
                ) : (
                  <View style={{ flex: 1, alignItems: "center", justifyContent: "center" }}>
                    <Ionicons name="medkit-outline" size={52} color="rgba(255,80,80,0.45)" />
                  </View>
                )}
              </View>
...
export default function DealsScreen(): React.ReactElement {
  ...
  const {
    ...
    inStock:  true,
    pageSize: 12,
    maxPages: 1,   // curated: exactly one page — feels editorial, not a full catalog
  });
  ...
  const Header = useMemo(() => (
    <>
      ...
      {/* Hero deal — first product enlarged */}
      {heroProd && (
...
const s = StyleSheet.create({
  ...
  liveDot:    { width: 7, height: 7, borderRadius: 4, backgroundColor: "#FCA5A5" },
  eyebrow:    { fontFamily: theme.fonts.bold, fontSize: 9.5, color: "rgba(255,255,255,0.60)", letterSpacing: 0.8 },
  ...
  timerLabel: { fontFamily: theme.fonts.semibold, fontSize: 11, color: "rgba(255,255,255,0.55)" },
  timerUnits: { flexDirection: "row", alignItems: "center", gap: 6 },
  digitWrap:  { alignItems: "center", gap: 4 },
  digitBox:   {
    borderRadius: 10, paddingHorizontal: 11, paddingVertical: 7, minWidth: 40,
    alignItems: "center", borderWidth: 1, borderColor: "rgba(255,255,255,0.18)",
  },
  digitValue: { fontFamily: theme.fonts.black, fontSize: 18, color: "#fff", letterSpacing: 1 },
  digitLabel: { fontFamily: theme.fonts.regular, fontSize: 9, color: "rgba(255,255,255,0.45)" },
  colon:      { fontFamily: theme.fonts.black, fontSize: 20, color: "rgba(255,255,255,0.45)", marginBottom: 12 },

  // Hero
  heroWrap:  { marginHorizontal: 12, marginTop: 16 },
  ...
  heroBadgeText: { fontFamily: theme.fonts.black, fontSize: 10, color: "#fff", letterSpacing: 0.4 },
  heroName:      { fontFamily: theme.fonts.black, fontSize: 15, color: "#fff", textAlign: "right", lineHeight: 22, letterSpacing: -0.2 },
  heroPriceRow:  { flexDirection: "row-reverse", alignItems: "baseline", gap: 4 },
  heroPrice:     { fontFamily: theme.fonts.black, fontSize: 24, color: "#FCA5A5", letterSpacing: -0.5 },
  heroCurrency:  { fontFamily: theme.fonts.bold,  fontSize: 13, color: "#FCA5A5" },
  heroSavings:   {
    flexDirection: "row-reverse", alignItems: "center", gap: 5,
    backgroundColor: "rgba(255,255,255,0.10)", borderRadius: 8,
    paddingHorizontal: 10, paddingVertical: 5, alignSelf: "flex-end",
  },
  heroSavingsText: { fontFamily: theme.fonts.semibold, fontSize: 11, color: "#FCA5A5" },

  // Section label between hero and grid
  sectionHeader: {
    flexDirection: "row-reverse", alignItems: "center", gap: 8,
    paddingHorizontal: 12, paddingTop: 20, paddingBottom: 4,
  },
