...
// Native feel easing: cubic-bezier(0.4, 0, 0.2, 1)
const EASING = [0.4, 0, 0.2, 1] as const;

export function LoadingOverlay({ readinessState, onRetry }: LoadingOverlayProps) {
  const { shouldShowOverlay, stage, error, retryCount, progress } = readinessState;
        ...
        >
          ...
          >
            ...
            >
              <StageIndicator
                label="Account"
                isComplete={readinessState.authReady}
                isActive={stage === LoadingStage.AUTH_CHECKING}
              />
              <StageIndicator
                label="Catalog"
                isComplete={readinessState.catalogReady}
                isActive={stage === LoadingStage.CATALOG_LOADING}
              />
              <StageIndicator
                label="Assets"
                isComplete={readinessState.assetsReady}
